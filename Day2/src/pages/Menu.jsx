...
export default function Menu({ items, HandleAddtocart, loading, categories, selected, HandleSelected, noOfPages, currentpage, Handlecurrentpage, search, HandleSearch }) {
    console.log(items)

    let pages = Array(noOfPages).fill(0).map((item, i) => i + 1)
        ...
        <div className='text-center mt-1.5 '>
            ...
            <div className='grid grid-cols-3 gap-1.5 '>
                <div className='col-span-1 ms-3 mt-7'>
                    ...
                    <Filter categories={categories} selected={selected} HandleSelected={HandleSelected} />
                </div>
                <div className='overflow-x-hidden col-span-2'>
                            ...
                            {items.map(item =>
                                <tr key={item.id}>
                                    ...
                                    <td className='text-center'>
                                        <Togglebutton handler={HandleAddtocart} id={item.id} isactive={item.isInCart} />
                                    </td>
                                </tr>
                            )}
                    ...
                    <nav aria-label="Page navigation example" className='mt-4 ms-10'>
                        <ul className="inline-flex -space-x-px text-sm">
                            ...
                            {noOfPages > 1 && pages.map((page) => (
                                <li>
                                    <button href="#" className={`flex items-center justify-center px-3 h-8 leading-tight border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${currentpage === page ? "bg-blue-500 text-white" : "bg-white text-gray-500"
